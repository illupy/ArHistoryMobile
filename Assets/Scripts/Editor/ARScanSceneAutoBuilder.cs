...
public class ARScanSceneAutoBuilder
{
    ...
    public static void BuildARScanSceneUI()
    {
        ...
        // Create Content Area and Text Panel to hold the scroll view
        GameObject contentArea = CreateUIObject("ContentArea", lessonPanel.transform);
        SetRect(contentArea.GetComponent<RectTransform>(), new Vector2(0.5f, 1f), new Vector2(0.5f, 1f), new Vector2(0f, -220f), new Vector2(900f, 300f));

        GameObject textPanel = CreatePanel(contentArea.transform, "TextPanel", new Color(0.12f, 0.13f, 0.22f, 0.95f), true);

        // Add ScrollRect and RectMask2D to TextPanel
        ...
        // Create StepContentText inside the TextPanel
        GameObject stepContent = CreateTMPText("StepContentText", textPanel.transform, "Nội dung bài học...", 28, FontStyles.Normal);
        TMP_Text stepTMP = stepContent.GetComponent<TMP_Text>();
        stepTMP.alignment = TextAlignmentOptions.Top; // Top-centered alignment for scroll readability

        RectTransform stepContentRT = stepContent.GetComponent<RectTransform>();
        stepContentRT.anchorMin = new Vector2(0f, 1f); // Stretch horizontally, anchor at the top
        stepContentRT.anchorMax = new Vector2(1f, 1f);
        stepContentRT.pivot = new Vector2(0.5f, 1f);   // Pivot at top-center
        stepContentRT.anchoredPosition = new Vector2(0f, -20f); // Top margin
        stepContentRT.sizeDelta = new Vector2(-40f, 0f); // -40 width offset (20px left & right padding)

        ContentSizeFitter fitter = stepContent.AddComponent<ContentSizeFitter>();
        fitter.horizontalFit = ContentSizeFitter.FitMode.Unconstrained;
        fitter.verticalFit = ContentSizeFitter.FitMode.PreferredSize;

        scrollRect.content = stepContentRT;
