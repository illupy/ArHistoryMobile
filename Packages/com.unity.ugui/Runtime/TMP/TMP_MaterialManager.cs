...
namespace TMPro
{
    ...
    public static class TMP_MaterialManager
    {
        ...
        internal static Material GetFallbackMaterial(TMP_FontAsset fontAsset, Material sourceMaterial, int atlasIndex)
        {
            ...
            if (fontAsset.atlasTextures == null || atlasIndex < 0 || atlasIndex >= fontAsset.atlasTextures.Length)
            {
                #if UNITY_EDITOR
                Debug.LogWarning($"[TMP Patch] GetFallbackMaterial: atlasIndex {atlasIndex} is out of bounds for font asset {fontAsset.name} (length: {fontAsset.atlasTextures?.Length ?? 0}). Using index 0 instead.");
                #endif
                atlasIndex = 0;
            }
